...
def solve_and_report(mapdl,shape_name=None,inputs=None):
...
# --- write CSV inside run folder
    ...
    mapdl.finish()
    return {
       "tot_def_max": tdef_max,
        "tot_def_avg": tdef_avg,
        "s1_max": s1_max,
        "s1_avg": s1_avg,
        "vm_max": seqv_max,
        "vm_avg": seqv_avg
...
def model_rectangular_beam(mapdl, length, width, height, pressure):
    ...
    return solve_and_report(mapdl,shape_name="rect",inputs=inputs)
...
def model_spherical_beam(mapdl, radius, pressure):
    ...
    inputs=[radius,0.0,0.0,pressure]
    return solve_and_report(mapdl, shape_name="sphere", inputs=inputs)
